                    ...
                    <table class="table-auto min-w-full rounded-xl">
                        <thead>
                            <tr class="bg-orange-100">
                                ...
                                <th scope="col"
                                    class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize">
                                    Date et Heure <br>de deplacement</th>
                                <th scope="col"
                                    class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize">
                                    Nombre de <br>passagers </th>
                                <th scope="col"
                                    class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize">
                                    Status du<br> traitement </th>
                                <th scope="col"
                                    class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize">
                                    Actions </th>
                            </tr>
                            ...
                            @foreach ($demandes->sortByDesc('id') as $i => $item)
                                <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                                    ...
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                        {{ $item->motif }}</td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                        {{ substr($item->lieu_depart, 0, 50) }}</td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                        {{ substr($item->destination, 0, 50) }}</td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                        {{ $item->date_deplacement }}</td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                        {{ $item->nbre_passagers }}</td>

                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">

                                        @if ($item->status == '1')
                                            <div
                                                class="py-1.5 px-2.5 bg-emerald-300 rounded-full flex justify-center w-20 items-center gap-1">
                                                <svg width="5" height="6" viewBox="0 0 5 6" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <circle cx="2.5" cy="3" r="2.5" fill="#059669">
                                                    </circle>
                                                </svg>
                                                <span class="font-medium text-xs text-emerald-50 ">Traitée</span>
                                            </div>
                                        @endif
                                        @if ($item->status == '0')
                                            <div
                                                class="py-1.5 px-2.5 bg-orange-300 rounded-full flex w-20 justify-center items-center gap-1">
                                                <svg width="5" height="6" viewBox="0 0 5 6" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <circle cx="2.5" cy="3" r="2.5" fill="#DC2626">
                                                    </circle>
                                                </svg>
                                                <span class="font-medium text-xs text-red-50 ">En attente</span>
                                            </div>
                                        @endif

                                        @if ($item->status == '2')
                                            <div
                                                class="py-1.5 px-2.5 bg-red-500 rounded-full flex w-20 justify-center items-center gap-1">
                                                <svg width="5" height="6" viewBox="0 0 5 6" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <circle cx="2.5" cy="3" r="2.5" fill="#DC2626">
                                                    </circle>
                                                </svg>
                                                <span class="font-medium text-xs text-red-50 ">Rejetée</span>
                                            </div>
                                        @endif
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                                                    ...
                                                    <li>
                                                        <a href="{{ route('demandes.show', $item->id) }}"
                                                            data-modal-target="show-modal"
                                                            data-modal-toggle="show-modal" onclick="show(event);"
                                                    ...
                                                    @if (Session::get('authUser')->hasRole('charroi'))
                                                        @if ($item->is_validated == 1 && $item->status == '0')
                                                            <li>
                                                                <a
                                                                    onclick="editdemande(event, {{ $item->id }}, {{ $vehicules }}, {{ $item->nbre_passagers }});"
                                                            ...
                                                            <li>
                                                                <a onclick="rejeter(event);"
                                                                    data-modal-target="rejet-modal"
